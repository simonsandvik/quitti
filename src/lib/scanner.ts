            ...
            try {
                ...
                if (result.confidence > 0) {
                    if (result.status === "FOUND") {
                        ...
                        // Auto-Fetch
                        // Enhanced Logic: Prefer PDF. If no PDF, check for images > 10KB. 
                        // If only small images (signatures/icons), ignore them and fall through to HTML save.
                        if (enrichedCandidate.hasAttachments && enrichedCandidate.attachments.length > 0) {
                            const pdf = enrichedCandidate.attachments.find(a => a.type.includes("pdf") || a.name.toLowerCase().endsWith(".pdf"));

                            // If no PDF, find a "good" image (size > 10KB to avoid icons/signatures)
                            let target = pdf;
                            if (!target) {
                                const largeImages = enrichedCandidate.attachments.filter(a =>
                                    (a.type.includes("image")) && a.size > 10000
                                );
                                if (largeImages.length > 0) {
                                    target = largeImages[0];
                                }
                            }
